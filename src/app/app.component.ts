...
export class AppComponent {
  ...
  isSupportTouchEvent = window.hasOwnProperty('ontouchstart');
  canvas;

  constructor(private readonly elementRef: ElementRef) {
    this.options.el = this.elementRef.nativeElement;
    this.init(this.options);
  }

  init(options) {
    this.createContainer(options.el);
    this.renderCanvas();
  }
  ...
  renderCanvas() {
    ...
    canvas.className = 'canvas'; //s.canvas;
  ...
  onTouchMove(e) {
    if (!this.isSupportTouchEvent && (e.which !== 1 || e.buttons === 0)) return; // pc canvas超出边界处理
    this.touchPoints(e);
    let touch = (e.touches && e.touches[0]) || e,
  ...
  onTouchEnd() {
    this.moveDreaw(this.isSupportTouchEvent ? this.inertialShift() : 0);
